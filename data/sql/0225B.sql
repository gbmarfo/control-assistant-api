...
DECLARE @rpPeriod DATE = '2025-04-11'
DECLARE @rpProject VARCHAR(10) = '88421'
DECLARE @rpPeriodType VARCHAR(10) = 'Weekly'
DECLARE @rpPIEPCCC VARCHAR(MAX) = '3,5,6' -- Modified to accept comma-separated values
...
BEGIN
	...
	-->> There is a possibility that multiple work packages will have the same BR_L2WorkPackage coding.
	;WITH a AS
	(
		SELECT
			aa.Snapshot_Date,
	...
	INNER JOIN NPDW_Report.sabi.v_fact_0200c_ActivityL2_Daily aa
		...
		AND aa.BR_PIEPCCC IN (SELECT value FROM STRING_SPLIT(@rpPIEPCCC, ',')) -- Modified
		AND aa.BR_ProjectNumber IN (@rpProject)
